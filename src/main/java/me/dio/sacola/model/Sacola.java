...
public class Sacola {
    ...
    @Id  //essa anotação indica a Primary Key (identificador unico) na tabela relacional
    @GeneratedValue(strategy = GenerationType.AUTO)  // essa anotação indica a forma de criação e a estrategia da PK
    private Long id;

    @ManyToOne(fetch = FetchType.LAZY, optional = false)  //essa anotação indica que um cliente pode ter varias sacolas
    @JsonIgnore
